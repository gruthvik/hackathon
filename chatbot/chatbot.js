...
if (progressButton) {
  progressButton.addEventListener("click", handleProgressView);
}

  async function handleProgressView() {
  try {
    console.log(username,sessionname);
    const res = await fetch(`${backendUrl}/get_portion_by_session`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const data = await res.json();
    const portionData = data.portion;

    if (!portionData || Object.keys(portionData).length === 0) {
      alert("No progress data available yet!");
      return;
    }
    ...
    title.textContent = "📘 Learning Progress Overview";
    ...
    // Build collapsible sections for each Unit
    Object.entries(portionData).forEach(([unit, topics]) => {
      const unitDiv = document.createElement("div");
      unitDiv.style.background = "#1e1e1e";
      unitDiv.style.borderRadius = "10px";
      unitDiv.style.marginBottom = "10px";
      unitDiv.style.padding = "10px";
      unitDiv.style.transition = "0.3s";
      ...
      topicContainer.style.marginTop = "8px";
      topicContainer.style.display = "none"; // hidden by default

      // Add topics
      Object.entries(topics).forEach(([topic, status]) => {
        const topicRow = document.createElement("div");
        topicRow.style.display = "flex";
        topicRow.style.justifyContent = "space-between";
        topicRow.style.alignItems = "center";
        topicRow.style.margin = "5px 0";
        topicRow.style.padding = "6px 10px";
        topicRow.style.borderRadius = "8px";
        topicRow.style.background = "#2a2a2a";
      ...
      unitHeader.addEventListener("click", () => {
        const open = topicContainer.style.display === "block";
        topicContainer.style.display = open ? "none" : "block";
        unitHeader.querySelector("span:last-child").textContent = open ? "▼" : "▲";
      });

      unitDiv.appendChild(unitHeader);
      unitDiv.appendChild(topicContainer);
    ...
    closeBtn.addEventListener("mouseover", () => (closeBtn.style.background = "#e64545"));
    closeBtn.addEventListener("mouseout", () => (closeBtn.style.background = "#FF5252"));
    ...
    alert("Failed to load progress data. Try again later.");
  }
}
